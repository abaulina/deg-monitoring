namespace CryptographyLib.Helpers
{
    public class Stribog
    {
        ...
        public Stribog() : this(256) { }

        public Stribog(int outputLength)
        {
            if (outputLength == 512)
            {
                for (var i = 0; i < 64; i++)
        ...
        private byte[] AddModulo512(byte[] a, byte[] b)
        {
            ...
            int i = 0, t = 0;
            var tempA = new byte[64];
            var tempB = new byte[64];
        ...
        private byte[] L(byte[] state)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                var tempBits1 = new System.Collections.BitArray(tempArray);
        ...
        private byte[] G_n(byte[] n, byte[] h, byte[] m)
        {
            ...
            var t = E(k, m);
            t = AddXor512(t, h);
            var newh = AddXor512(t, m);
            return newh;
        }

        public byte[] ComputeHash(byte[] message)
        {
            message = message.Reverse().ToArray(); // from little endian to big endian

            var paddedMes = new byte[64];
            ...
            var n512 = BitConverter.GetBytes(512);
            var inc = 0;
            while (len >= 512)
            {
                inc++;
                var tempMes = new byte[64];
            ...
            var mesLen = BitConverter.GetBytes(message1.Length * 8);
